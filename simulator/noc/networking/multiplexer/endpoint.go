...
func (ep *EndPoint) PlugIn(port akita.Port, srcBufCap int) {
	if ep.DevicePort != nil {
		panic("endpoint can only connect to one device port")
	}
...
func (ep *EndPoint) sendFlitOut(now akita.VTimeInSec) bool {
	...
	if err == nil {
		...
		if len(ep.flitsToSend) == 0 {
			ep.DevicePort.NotifyAvailable(now)
		}
		return true
	}
	return false
...
// WithDevicePort sets a list of ports that communicate directly through the
// End Point.
func (b EndPointBuilder) WithDevicePort(port akita.Port) EndPointBuilder {
	b.devicePort = port
	return b
}
...
func (b EndPointBuilder) Build(name string) *EndPoint {
	...
	ep.PlugIn(b.devicePort, 2*b.numReqPerCycle)
