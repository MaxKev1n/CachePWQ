...
func (c *coalescer) Tick(now akita.VTimeInSec) bool {
	c.processingMMU = !c.processingMMU

	if c.processingMMU {
		return c.processMMU(now)
	} else {
		return c.processData(now)
	}
}

func (c *coalescer) processMMU(now akita.VTimeInSec) bool {
	req := c.cache.MMUPort.Peek()
	if req == nil {
		return false
	}

	return c.processReq(now, req.(mem.AccessReq), c.cache.MMUPort)
}

func (c *coalescer) processData(now akita.VTimeInSec) bool {
	req := c.cache.TopPort.Peek()
	if req == nil {
		return false
	}

	return c.processReq(now, req.(mem.AccessReq), c.cache.TopPort)
}
...
func (c *coalescer) processReq(
	...
	if c.isReqLastInWave(req) {
		if len(c.toCoalesce) == 0 || c.canReqCoalesce(req) {
			return c.processReqLastInWaveCoalescable(now, req, srcPort)
		}
		return c.processReqLastInWaveNoncoalescable(now, req, srcPort)
	}

	if len(c.toCoalesce) == 0 || c.canReqCoalesce(req) {
		return c.processReqCoalescable(now, req, srcPort)
	}
	return c.processReqNoncoalescable(now, req, srcPort)
}
...
func (c *coalescer) coalesceRead() *transaction {
	...
	coalescedRead := mem.ReadReqBuilder{}.
		...
		WithInfo(c.toCoalesce[0].read.Info).
		Build()
	return &transaction{
