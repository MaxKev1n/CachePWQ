...
type HierarchicalMemSideCaPWQGPUBuilder struct {
	*CommonBuilder

	// specific componenets
}

func MakeHierarchicalMemSideCaPWQGPUBuilder() HierarchicalMemSideCaPWQGPUBuilder {
	// TODO: should this be using new? is the object being allocated on the stack?
	cbp := CommonBuilder{}
	b := HierarchicalMemSideCaPWQGPUBuilder{CommonBuilder: &cbp}
...
func (b HierarchicalMemSideCaPWQGPUBuilder) Build(name string, id uint64) *mgpusim.GPU {
	...
	if mmuType != "CaPWQMMU" {
		panic("Only CaPWQMMU is supported in HierarchicalMemSideCaPWQGPUBuilder")
	}
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) createGlobalNoC(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) establishL1ToL2RoutingPath(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) establishTPC(chiplet *Chiplet) {
	maxCUsPerGPC := 16
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) establishGPC(chiplet *Chiplet) {
	maxCUsPerGPC := 16
	numGPCs := (len(chiplet.CUs)-1)/maxCUsPerGPC + 1
	numTPCs := numGPCs * (maxCUsPerGPC / 2) // 2 CUs per TPC
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) connectGlobalNoC(chiplet *Chiplet) {
	for i, mux := range chiplet.gpcMux {
		...
		numSAPerGPC := b.numShaderArrayPerChiplet / len(chiplet.gpcMux)

		for j := i * numSAPerGPC; j < (i+1)*numSAPerGPC; j++ {
			ep.PlugIn(chiplet.L1STLBs[j].GetBottomPort(), 4)
			ep.PlugIn(chiplet.L1ITLBs[j].GetBottomPort(), 4)
		}
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) establishL2TLBToL3TLBRoutingPath(chiplet *Chiplet) {
	singeLowModuleFinder := new(cache.SingleLowModuleFinder)
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) buildL2TLB(chiplet *Chiplet) {
	maxCUsPerGPC := 16 // same as NVIDIA A100
	numGPCs := ((len(chiplet.CUs) - 1) / maxCUsPerGPC) + 1
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) buildL3TLB(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) connectL3TLBTOMMU(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) setupInterchipNetwork() {
...
func (b *HierarchicalMemSideCaPWQGPUBuilder) InterChipletPorts(c *Chiplet) []akita.Port {
	ports := []akita.Port{
