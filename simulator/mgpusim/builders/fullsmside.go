...
type FullSMSideGPUBuilder struct {
	*CommonBuilder
...
func (b *FullSMSideGPUBuilder) WithNumL2TLBSlices(n int) {
	b.numL2TLBSlices = n
}

// Distributed TLB specific function

// MakeFullSMSideGPUBuilder provides a GPU builder that can builds MCM GPU.
func MakeFullSMSideGPUBuilder() FullSMSideGPUBuilder {
	// TODO: should this be using new? is the object being allocated on the stack?
	cbp := CommonBuilder{}
	b := FullSMSideGPUBuilder{CommonBuilder: &cbp}
	b.SetDefaultCommonBuilderParams()
	return b
}

func (b FullSMSideGPUBuilder) Build(name string, id uint64) *mgpusim.GPU {
	b.createGPU(name, id)
...
func (b *FullSMSideGPUBuilder) BuildSAs(chiplet *Chiplet) {
	...
	saBuilder.withConfig("FullSMSide")
...
func (b *FullSMSideGPUBuilder) buildL2TLB(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) connectCP() {
...
func (b *FullSMSideGPUBuilder) createL1ToL2NoC(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) createL1TLBToL2TLBNoC(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) connectL1ToL2NoC(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) connectL1TLBToL2TLBNoC(chiplet *Chiplet) {
	...
	for i := 0; i < b.numL2TLBSlices; i++ {
		...
		chiplet.L1TLBToL2TLBNoC.PlugInMemSide(
			chiplet.L2TLBs[i].(*tlb.SMSideTLB).RemoteTopPort,
			64,
			1,
		)
	...
	for i, l1vTLB := range chiplet.L1VTLBs {
		...
		chiplet.L1TLBToL2TLBNoC.PlugInSMSide(l1vTLB.(*tlb.SMSideL1TLB).RemotePort, 16, 1)
	...
	for i, l1iTLB := range chiplet.L1ITLBs {
		...
		chiplet.L1TLBToL2TLBNoC.PlugInSMSide(l1iTLB.(*tlb.SMSideL1TLB).RemotePort, 16, 1)
	...
	for i, l1sTLB := range chiplet.L1STLBs {
		...
		chiplet.L1TLBToL2TLBNoC.PlugInSMSide(l1sTLB.(*tlb.SMSideL1TLB).RemotePort, 16, 1)
...
func (b *FullSMSideGPUBuilder) setupInterchipNetwork() {
...
func (b *FullSMSideGPUBuilder) InterChipletPorts(c *Chiplet) []akita.Port {
	ports := []akita.Port{
...
func (b *FullSMSideGPUBuilder) buildMMU(chiplet *Chiplet) {
	if yamlconfig.OverrideConfig == nil {
		...
		case "MPWMMU":
			panic("FullSMSideGPUBuilder does not support MPWMMU yet")
		case "BaselineMMU":
			b.buildDefaultMMU(chiplet)
		default:
...
func (b *FullSMSideGPUBuilder) buildIdealMMU(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) buildCAPWQMMU(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) buildDefaultMMU(chiplet *Chiplet) {
	maxNumReqInFlight := 16
...
func (b *FullSMSideGPUBuilder) connectL2TLBTOMMU(chiplet *Chiplet) {
...
func (b *FullSMSideGPUBuilder) connectMMUToL1(chiplet *Chiplet) {
	numL1VCachePerMMU := len(chiplet.L1VCaches) / b.numL2TLBSlices
	if len(chiplet.L1VCaches)%b.numL2TLBSlices != 0 {
		log.Panicf("numL1VCaches %d is not divisible by numL2TLBSlices %d",
			len(chiplet.L1VCaches), b.numL2TLBSlices)
	}
	for i, mmu := range b.MMUs {
		l1ToMMUConn := akita.NewDirectConnection(
			fmt.Sprintf("%s.MMU-L1VCacheConn[%d]", chiplet.name, i),
			b.engine, b.freq,
		)

		lowModuleFinder := cache.NewInterleavedLowModuleFinder(1 << b.log2CacheLineSize)

		mmu.SetLowModuleFinder(lowModuleFinder)

		for j := 0; j < numL1VCachePerMMU; j++ {
			l1vCache := chiplet.L1VCaches[i*numL1VCachePerMMU+j].(*smside.Cache)
