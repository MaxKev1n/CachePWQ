...
type HierarchicalMemSideDistTLBGPUBuilder struct {
	*CommonBuilder

	// specific componenets
}

func MakeHierarchicalMemSideDistTLBGPUBuilder() HierarchicalMemSideDistTLBGPUBuilder {
	// TODO: should this be using new? is the object being allocated on the stack?
	cbp := CommonBuilder{}
	b := HierarchicalMemSideDistTLBGPUBuilder{CommonBuilder: &cbp}
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) createGlobalNoC(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) establishL1ToL2RoutingPath(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) establishTPC(chiplet *Chiplet) {
	maxCUsPerGPC := 16
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) establishGPC(chiplet *Chiplet) {
	maxCUsPerGPC := 16
	numGPCs := (len(chiplet.CUs)-1)/maxCUsPerGPC + 1
	numTPCs := numGPCs * (maxCUsPerGPC / 2) // 2 CUs per TPC
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) establishL2Partition(chiplet *Chiplet) {
	maxL2PerPartition := 8
	...
	for i, l2tlb := range chiplet.L2TLBs {
		...
		localPort := mux.AddLowSidePort(ep)
		mux.AddRoute(l2tlb.GetTopPort(), localPort)
	}
}

func (b *HierarchicalMemSideDistTLBGPUBuilder) connectGlobalNoC(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) establishL1TLBToL2TLBRoutingPath(chiplet *Chiplet) {
	numCUsPerGPC := 16
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) connectMMUToGlobalNoC(chiplet *Chiplet) {
	chiplet.MMU.SetLowModuleFinder(chiplet.lowModuleFinderForL1)
}

func (b *HierarchicalMemSideDistTLBGPUBuilder) buildMemBanks(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) connectL2TLBTOMMU(chiplet *Chiplet) {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) setupInterchipNetwork() {
...
func (b *HierarchicalMemSideDistTLBGPUBuilder) InterChipletPorts(c *Chiplet) []akita.Port {
	ports := []akita.Port{
