...
def plot_normalized_time(
    ...
    baseline_1024: pd.DataFrame,
    opt: pd.DataFrame,
    opt_1024: pd.DataFrame,
    out_dir: str,
) -> None:
    """
    ...
    opt_1024["Data"] = [
        (
            baseline_1024["Data"][i] / opt_1024["Data"][i]
            if opt_1024["Data"][i] != 0 and baseline_1024["Data"][i] != 0
            else 0
        )
        for i in range(len(benchmarks))
    ]
    ...
    opt_1024 = pd.concat(
        [
            ...
            pd.DataFrame(
                {
                    "Benchmark": ["Ave."],
                    "Data": [harmonic_mean(opt_1024["Data"])],
                }
            ),
        ],
        ignore_index=True,
    ...
    print("opt_1024 Data:", opt_1024["Data"].tolist())

    bar1 = plt.bar(
        r1,
        opt["Data"],
        width=bar_width,
        label="512 Entries L2 TLB",
        color="#C3D9F1",
        edgecolor="black",
        linewidth=1.5,
    )
...
if __name__ == "__main__":
    ...
    baseline_1024 = pd.DataFrame(columns=["Benchmark", "Data"])
    opt = pd.DataFrame(columns=["Benchmark", "Data"])
    opt_1024 = pd.DataFrame(columns=["Benchmark", "Data"])
    ...
    for benchmark in get_high_mpki_benchmarks():
        ...
        baseline_1024 = append_row(baseline_1024, benchmark, "../../final_final_data/baseline_1024L2TLB")
        opt = append_row(opt, benchmark, "../../final_final_data/nbwalker-full")
        opt_1024 = append_row(opt_1024, benchmark, "../../final_final_data/nbwalkerfull_1024L2TLB")

    plot_normalized_time(
        baseline=baseline,
        baseline_1024=baseline_1024,
        opt=opt,
        opt_1024=opt_1024,
        out_dir=args.outDir,
    )