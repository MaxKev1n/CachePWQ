...
def collect_performance_data(benchmark_name: str, input_dir: str) -> float:
    ...
    for _, row in df.iterrows():
        ...
        elif " ptw-read" in row.iloc[2]:
            all_data += row.iloc[3]
            
    return 1 - (performance_data / all_data)
...
def plot_normalized_time(
    ...
    bar_width = 0.1
    r1 = np.arange(len(benchmarks) + 1) * (1 * bar_width + 0.2)
    ...
    plt.ylabel("Ratio of L2 req.\nfrom L1 MSHR", fontsize=36, fontweight="bold")
    plt.yticks(
